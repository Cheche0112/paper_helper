...
"""
    ...
    async def translate(self, langto="zh-CN"):
        try:
            # 设置 20 秒的超时
            self.title_translated = await asyncio.wait_for(async_translate(self.title, langto=langto), timeout=20.0)
        except asyncio.TimeoutError:
            # 处理超时情况
            self.title_translated = ""  # 或者其他适当的处理方式
            # print("Translation timed out after 20 seconds")
